...
function forecast(latitude, longitude, date) {
  return fetch(`https://se-weather-api.herokuapp.com/api/v1/forecast?latitude=${latitude}&longitude=-${longitude}&date=${date}`).then(response => {
    return response.json();
  });
}

function geoLocation(zipCode) {
  return fetch(`https://se-weather-api.herokuapp.com/api/v1/geo?zip_code=${zipCode}`).then(response => {
    return response.json();
  });
}
...
(0, _utils.geoLocation)('90210').then(result => {
  document.getElementById("cityName").innerHTML = result.city;
  return (0, _utils.forecast)(result.latitude, result.longitude, '01/24/2020');
}).then(result => {
  ...
  document.getElementById("weeklySummary").innerHTML = result.daily.summary; // TODO: Images not working for some reason, need to debug

  document.getElementById("todayImage").src = "img/" + result.daily.data[0].icon + ".png";
  document.getElementById("todaySummary").innerHTML = result.daily.data[0].summary; // TODO: Round temperatures to whole number
...
},{"./utils":"src/utils.js"}],"../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
